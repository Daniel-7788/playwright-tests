...
test('訊息發送流程測試全部發送', async ({ page, context }) => {
  try {
    ...
    const activityName = `ci_test_send_all_${dateStr} ${timeStr}`;
    ...
    // 選擇群發
    await page.locator('label:has-text("會員群發")').click();
    console.log('👥 選擇會員群發');
